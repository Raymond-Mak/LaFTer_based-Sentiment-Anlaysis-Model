...
# 导入共享组件
#from .lafter_common import load_clip_to_cpu, BaseLaFTerTrainer
...
class LaFTerUFT(nn.Module):
    ...
    def txt_features_for_text_cls(self):
        ...
        if self.txt_cls in ['cls_only', 'templates_only', 'lafter']:
            ...
            if os.path.isfile(cache_filename):
                ...
                try:
                    ...
                    if isinstance(cached_data, dict) and 'features' in cached_data and 'labels' in cached_data:
                        ...
                        print("Detected old cache format, regenerating for consistency...")
            ...
            if zeroshot_weights.shape[0] != labels_for_descriptions.shape[0]:
                ...
                print(f"Feature count: {zeroshot_weights.shape[0]}")
                print(f"Label count: {labels_for_descriptions.shape[0]}")
                ...
                print(f"Resized to consistent size: {min_size}")
                ...
                print(f"Corrected data saved to: {cache_filename}")
            
            print("=== Returning data statistics ===")
            print(f"Final feature shape: {zeroshot_weights.shape}")
            print(f"Final label shape: {labels_for_descriptions.shape}")
    ...
    def _generate_new_embeddings(self, desc, labels_for_descriptions, cache_filename):
        ...
        # 确保标签是张量格式
        labels_for_descriptions = torch.tensor(labels_for_descriptions).cuda()
        print(f"Initial label count: {len(labels_for_descriptions)}")
        ...
        with torch.no_grad():
            ...
            # 验证数量一致性
            print(f"Number of generated features: {zeroshot_weights.shape[0]}")
            print(f"Corresponding labels: {processed_labels.shape[0]}")
    ...
    def forward_supervised(self, x):
        """监督学习前向方法，处理图像并生成预测 logits"""
        img_features = self.incorporate_prompt(x)  # 包含提示的图像特征
        img_features = self.embeddings_after_prompts(img_features)  # 处理后的特征
        img_features_adapter = self.adapter(img_features)  # 通过适配器生成 logits
        return img_features_adapter

    def forward_dual(self, x):
        """双头前向方法，同时返回分类头与分布学习头的logits"""
        img_features = self.incorporate_prompt(x)
        img_features = self.embeddings_after_prompts(img_features)
        cls_logits = self.adapter(img_features)
        dist_logits = self.dist_head(img_features)
...
class LaFTer(BaseLaFTerTrainer):
